...
function savedView(): 'list' | 'grid' {
  try {
    return storage()?.getItem('zhou-articles-view') === 'grid' ? 'grid' : 'list';
  } catch {
    return 'list';
  }
}
...
export function initArticleDiscovery(root: ParentNode = document): Cleanup {
  ...
  const setView = (next: 'list' | 'grid', persist = true) => {
    ...
    if (persist)
      try {
        storage()?.setItem('zhou-articles-view', next);
      } catch {
        /* storage is optional */
      }
  };
  const render = () => {
    const tokens = normalizeArticleText(search?.value ?? '')
      .split(' ')
      .filter(Boolean);
    let visible = 0;
    for (const card of cards) {
      ...
      try {
        ...
        if (Array.isArray(encodedTags) && encodedTags.every((tag) => typeof tag === 'string'))
          cardTags = new Set(encodedTags);
      } catch {
        /* invalid markup does not match selected tags */
      }
  ...
  const onTagClick = (event: Event) => {
    ...
    const tag = button.dataset.articleTag;
    if (!tag) return;
  ...
  const onTagKeydown = (event: KeyboardEvent) => {
    const index = tags.indexOf(event.currentTarget as HTMLButtonElement);
    if (index < 0) return;
    const target =
      event.key === 'Home'
        ? 0
        : event.key === 'End'
          ? tags.length - 1
          : event.key === 'ArrowLeft'
            ? (index - 1 + tags.length) % tags.length
  ...
  const onViewClick = (event: Event) =>
    setView((event.currentTarget as HTMLButtonElement).dataset.articleView === 'grid' ? 'grid' : 'list');
  const onReset = () => {
    if (search) search.value = '';
    selected.clear();
    for (const tag of tags) tag.setAttribute('aria-pressed', 'false');
    render();
    search?.focus();
  };
  ...
  for (const tag of tags) {
    ...
    tag.addEventListener('keydown', onTagKeydown);
  }
  ...
  const cleanup = () => {
    ...
    for (const tag of tags) {
      ...
      tag.removeEventListener('keydown', onTagKeydown);
    }
  ...
  discoveryCleanups.set(root, cleanup);
  return cleanup;
}
...
export function initArticleReader(root: ParentNode = document, options: ReaderOptions = {}): Cleanup {
  ...
  const update = () => {
    frame = undefined;
    if (!progress) return;
    const rect = (content ?? reader).getBoundingClientRect();
    const total = rect.height - window.innerHeight;
    progress.value = total <= 0 ? 100 : Math.min(100, Math.max(0, (-rect.top / total) * 100));
  };
  const schedule = () => {
    if (frame === undefined) frame = requestFrame(update);
  };
  ...
  const copy = async (code: HTMLElement) => {
    try {
      ...
      await adapter.writeText(code.textContent ?? '');
      if (status) status.textContent = '代码已复制。';
    } catch {
      if (status) status.textContent = '无法复制代码。';
    }
  };
  ...
  readerCleanups.set(root, cleanup);
  return cleanup;
}
