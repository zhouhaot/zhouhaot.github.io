...
describe('article route structure', () => {
  it('keeps the article view model at build time and uses Astro supplied headings', () => {
    const detail = readFileSync(resolve('src/pages/articles/[id].astro'), 'utf8'); const domain = readFileSync(resolve('src/domain/articles.ts'), 'utf8');
    expect(detail).toMatch(/await render\(entry\)/); expect(detail).toContain('headings'); expect(detail).toContain('buildArticleToc(headings)');
