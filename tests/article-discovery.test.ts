...
describe('article discovery', () => {
  ...
  it('reveals a recoverable zero state and resets filters, focus, and cards without changing view', () => {
    document.body.innerHTML = markup;
    const cleanup = initArticleDiscovery();
    const search = document.querySelector<HTMLInputElement>('[data-article-search]')!;
    ...
    expect(document.querySelector<HTMLElement>('[data-article-discovery]')?.dataset.view).toBe('list');
    cleanup();
  });
  ...
  it('moves tag focus with keys without changing selected tags', () => {
    document.body.innerHTML = markup;
    initArticleDiscovery();
    const ai = document.querySelector<HTMLButtonElement>('[data-article-tag="ai"]')!;
    ai.focus();
  ...
  it('uses exclusive valid persisted views and tolerates invalid or throwing storage', () => {
    localStorage.setItem('zhou-articles-view', 'grid');
    document.body.innerHTML = markup;
    initArticleDiscovery();
    ...
    expect(() => initArticleDiscovery()).not.toThrow();
    expect(document.querySelector<HTMLElement>('[data-article-discovery]')?.dataset.view).toBe('list');
  });

  it('cleans earlier mounts before remounting', () => {
    document.body.innerHTML = markup;
    const first = initArticleDiscovery();
    const second = initArticleDiscovery();
    document.querySelector<HTMLButtonElement>('[data-article-tag="ai"]')!.click();
    expect(document.querySelector('[data-article-tag="ai"]')?.getAttribute('aria-pressed')).toBe('true');
    first();
    second();
  });
});
